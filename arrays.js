...
// Make a variable called arrayLength and set it equal to the length of
// "sampleArray". Then, log it to the console.
...
for(let b = 0 ; b < originalArray.length; b++){
    reversedArray.unshift(originalArray[b])
}
...
for(let v = 0; v<groceryList.length; v++){
    if(groceryList[v] == itemToRemove){
        let spliced = groceryList.splice(v,1)
...
for(let y = 0; y<repeats.length;y++){
    if(repeats[y] == repeats[y+1]){
        sum = sum + repeats[y]
    }
} console.log(sum)
